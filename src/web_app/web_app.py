...
cred, _ = default()
firestore_db = firestore.Client(project=os.getenv("project_id"), credentials=cred)
logger.info("Connection established with Firestore Database.")
...
def index():
    if request.method == "POST":
        ...
        firestore_db.collection("UserQueryPosts").add(
            {
                "user_query_text": prompt,
                "user_query_top_k": num_posts,
                "posts": result,
            }
        )
        logger.info("The result as been saved on Firestore Database.")
